#!/bin/bash
set -u
H=$RUN_HOST; D=~/Desktop/august19/run-rep1; mkdir -p "$D/logs"
rsync -az --no-perms $H:'/var/tmp/lab/bvp-rep1/{results.tsv,blackboard.md,MISTAKES.md,LEARNINGS.md,DESIRES.md}' "$D/" 2>/dev/null
L=$(ssh $H 'ls -d /var/tmp/labruns/bvp-rep1-*/ 2>/dev/null | tail -1' 2>/dev/null)
[ -n "$L" ] && rsync -az $H:"$L" "$D/logs/" 2>/dev/null
rsync -az $H:'/var/tmp/labruns/bvp-rep1-*.json' "$D/" 2>/dev/null
